...
// HB - 05/05/05
// ----------------------------------------------------------------
#ifndef _FACEQUAD9_H_
#define _FACEQUAD9_H_
...
class FaceQuad9: public FaceElement {
  private:
        int Nodes[9];
        static double RefCoords[9][2]; // coords of the nodes in the ref./parametric domain

  public:
        enum { NumberOfNodes=9 };
...
template<typename Scalar>
void
FaceQuad9::GetShapeFctVal(Scalar *Shape, Scalar *m)
{
...
template<typename Scalar>
void
FaceQuad9::GetdShapeFct(Scalar *dShapex, Scalar *dShapey, Scalar *m)
{
...
template<typename Scalar>
void
FaceQuad9::Getd2ShapeFct(Scalar *d2Shapex, Scalar *d2Shapey, Scalar *d2Shapexy, Scalar *m)
{
  ...
  d2Shapex[8] = -2*yym;
  d2Shapex[0] = onequart*(ym*2 + d2Shapex[8]);
  d2Shapex[1] = onequart*(ym*2 + d2Shapex[8]);
  d2Shapex[2] = onequart*(yp*2 + d2Shapex[8]);
  d2Shapex[3] = onequart*(yp*2 + d2Shapex[8]);
  d2Shapex[4] = -ym - onehalf*d2Shapex[8];
  d2Shapex[5] =     - onehalf*d2Shapex[8];
  d2Shapex[6] = -yp - onehalf*d2Shapex[8];
  d2Shapex[7] =     - onehalf*d2Shapex[8];

  d2Shapey[8] = -2*xxm;
  d2Shapey[0] = onequart*(xm*2 + d2Shapey[8]);
  d2Shapey[1] = onequart*(xp*2 + d2Shapey[8]);
  d2Shapey[2] = onequart*(xp*2 + d2Shapey[8]);
  d2Shapey[3] = onequart*(xm*2 + d2Shapey[8]);
  d2Shapey[4] =     - onehalf*d2Shapey[8];
  d2Shapey[5] = -xp - onehalf*d2Shapey[8];
  d2Shapey[6] =     - onehalf*d2Shapey[8];
  d2Shapey[7] = -xm - onehalf*d2Shapey[8];

  d2Shapexy[8] = 4*x*y;
  d2Shapexy[0] = onequart*( 1-2*x-2*y + d2Shapexy[8]);
  d2Shapexy[1] = onequart*(-1-2*x+2*y + d2Shapexy[8]);
  d2Shapexy[2] = onequart*( 1+2*x+2*y + d2Shapexy[8]);
  d2Shapexy[3] = onequart*(-1+2*x-2*y + d2Shapexy[8]);
  d2Shapexy[4] =  x - onehalf*d2Shapexy[8];
  d2Shapexy[5] = -y - onehalf*d2Shapexy[8];
  d2Shapexy[6] = -x - onehalf*d2Shapexy[8];
  d2Shapexy[7] =  y - onehalf*d2Shapexy[8];
}

template<typename Scalar>
void
FaceQuad9::Getd3ShapeFct(Scalar *d3Shapex, Scalar *d3Shapey, Scalar *d3Shapex2y, Scalar *d3Shapexy2, Scalar *m)
{
  ...
  d3Shapex2y[0] = -onehalf + y;
  d3Shapex2y[1] = -onehalf + y;
  d3Shapex2y[2] =  onehalf + y;
  d3Shapex2y[3] =  onehalf + y;
  d3Shapex2y[4] =  1 - 2*y;
  d3Shapex2y[5] =    - 2*y;
  d3Shapex2y[6] = -1 - 2*y;
  d3Shapex2y[7] =    - 2*y;
  ...
  d3Shapexy2[0] = -onehalf + x;
  d3Shapexy2[1] =  onehalf + x;
  d3Shapexy2[2] =  onehalf + x;
  d3Shapexy2[3] = -onehalf + x;
  d3Shapexy2[4] =    - 2*x;
  d3Shapexy2[5] = -1 - 2*x;
  d3Shapexy2[6] =    - 2*x;
  d3Shapexy2[7] =  1 - 2*x;
...
template<typename Scalar, typename CoordSetT>
void
FaceQuad9::LocalToGlobalCoord(Scalar *M, Scalar *m, CoordSetT &cs)
{
...
template<typename Scalar, typename CoordSetT>
void
FaceQuad9::ComputedMdxAnddMdy(Scalar *dMdx, Scalar *dMdy, Scalar *m, CoordSetT &cs)
{
...
template<typename Scalar, typename CoordSetT>
void
FaceQuad9::Computed2Mdx2d2Mdy2Andd2Mdxdy(Scalar *d2Mdx2, Scalar *d2Mdy2, Scalar *d2Mdxdy, Scalar *m, CoordSetT &cs)
{
  ...
  Scalar d2Shapex[9], d2Shapey[9], d2Shapexy[9];
  ...
  for(int i=0; i<9; i+=3) {
    d2Mdx2[0] += d2Shapex[i]*X[i] + d2Shapex[i+1]*X[i+1] + d2Shapex[i+2]*X[i+2];
    d2Mdx2[1] += d2Shapex[i]*Y[i] + d2Shapex[i+1]*Y[i+1] + d2Shapex[i+2]*Y[i+2];
    d2Mdx2[2] += d2Shapex[i]*Z[i] + d2Shapex[i+1]*Z[i+1] + d2Shapex[i+2]*Z[i+2];

    d2Mdy2[0] += d2Shapey[i]*X[i] + d2Shapey[i+1]*X[i+1] + d2Shapey[i+2]*X[i+2];
    d2Mdy2[1] += d2Shapey[i]*Y[i] + d2Shapey[i+1]*Y[i+1] + d2Shapey[i+2]*Y[i+2];
    d2Mdy2[2] += d2Shapey[i]*Z[i] + d2Shapey[i+1]*Z[i+1] + d2Shapey[i+2]*Z[i+2];

    d2Mdxdy[0] += d2Shapexy[i]*X[i] + d2Shapexy[i+1]*X[i+1] + d2Shapexy[i+2]*X[i+2];
    d2Mdxdy[1] += d2Shapexy[i]*Y[i] + d2Shapexy[i+1]*Y[i+1] + d2Shapexy[i+2]*Y[i+2];
    d2Mdxdy[2] += d2Shapexy[i]*Z[i] + d2Shapexy[i+1]*Z[i+1] + d2Shapexy[i+2]*Z[i+2];
  }
}

template<typename Scalar, typename CoordSetT>
void
FaceQuad9::Computed3Mdx3d3Mdy3d3Mdx2dyAndd3Mdxdy2(Scalar *d3Mdx3, Scalar *d3Mdy3, Scalar *d3Mdx2dy, Scalar *d3Mdxdy2, Scalar *m, CoordSetT &cs)
{
  ...
  Scalar d3Shapex[9], d3Shapey[9], d3Shapex2y[9], d3Shapexy2[9];
  ...
  for(int i=0; i<9; i+=3) {
/* all zero
    d3Mdx3[0] += d3Shapex[i]*X[i] + d3Shapex[i+1]*X[i+1] + d3Shapex[i+2]*X[i+2];
    d3Mdx3[1] += d3Shapex[i]*Y[i] + d3Shapex[i+1]*Y[i+1] + d3Shapex[i+2]*Y[i+2];
    d3Mdx3[2] += d3Shapex[i]*Z[i] + d3Shapex[i+1]*Z[i+1] + d3Shapex[i+2]*Z[i+2];

    d3Mdy3[0] += d3Shapey[i]*X[i] + d3Shapey[i+1]*X[i+1] + d3Shapey[i+2]*X[i+2];
    d3Mdy3[1] += d3Shapey[i]*Y[i] + d3Shapey[i+1]*Y[i+1] + d3Shapey[i+2]*Y[i+2];
    d3Mdy3[2] += d3Shapey[i]*Z[i] + d3Shapey[i+1]*Z[i+1] + d3Shapey[i+2]*Z[i+2];
*/
    d3Mdx2dy[0] += d3Shapex2y[i]*X[i] + d3Shapex2y[i+1]*X[i+1] + d3Shapex2y[i+2]*X[i+2];
    d3Mdx2dy[1] += d3Shapex2y[i]*Y[i] + d3Shapex2y[i+1]*Y[i+1] + d3Shapex2y[i+2]*Y[i+2];
    d3Mdx2dy[2] += d3Shapex2y[i]*Z[i] + d3Shapex2y[i+1]*Z[i+1] + d3Shapex2y[i+2]*Z[i+2];

    d3Mdxdy2[0] += d3Shapexy2[i]*X[i] + d3Shapexy2[i+1]*X[i+1] + d3Shapexy2[i+2]*X[i+2];
    d3Mdxdy2[1] += d3Shapexy2[i]*Y[i] + d3Shapexy2[i+1]*Y[i+1] + d3Shapexy2[i+2]*Y[i+2];
    d3Mdxdy2[2] += d3Shapexy2[i]*Z[i] + d3Shapexy2[i+1]*Z[i+1] + d3Shapexy2[i+2]*Z[i+2];
  }
}

template<typename Scalar, typename CoordSetT>
Scalar
FaceQuad9::GetJacobian(Scalar *m, CoordSetT &cs)
{
  // Compute ∂M/∂ξ and ∂M/∂η
...
template<typename Scalar, typename CoordSetT>
Scalar
FaceQuad9::GetShapeFctAndJacobian(Scalar *Shape, Scalar *m, CoordSetT &cs)
{
  GetShapeFctVal(Shape, m);
...
template<typename Scalar, typename CoordSetT>
void
FaceQuad9::GetIsoParamMappingNormalJacobianProduct(Scalar *JNormal, Scalar *m, CoordSetT &cs)
{
...
template<typename Scalar, typename CoordSetT>
void
FaceQuad9::ComputedJNormaldxAnddJNormaldy(Scalar *dJNormaldx, Scalar *dJNormaldy, Scalar *m, CoordSetT &cs)
{
...
template<typename Scalar, typename CoordSetT>
void
FaceQuad9::Computed2JNormaldx2d2JNormaldy2Andd2JNormaldxdy(Scalar *d2JNormaldx2, Scalar *d2JNormaldy2, Scalar *d2JNormaldxdy, Scalar *m, CoordSetT &cs)
{
...
template<typename Scalar, typename CoordSetT>
Scalar
FaceQuad9::GetIsoParamMappingNormalAndJacobian(Scalar *Normal, Scalar *m, CoordSetT &cs)
{
...
template<typename Scalar, typename CoordSetT>
void
FaceQuad9::GetdJNormal(Scalar dJNormal[][3], Scalar* m, CoordSetT& cs)
{
...
template<typename Scalar, typename CoordSetT>
void
FaceQuad9::ComputeddJNormaldxAndddJNormaldy(Scalar ddJNormaldx[][3], Scalar ddJNormaldy[][3], Scalar* m, CoordSetT& cs)
{
...
template<typename Scalar, typename CoordSetT>
void
FaceQuad9::Getd2JNormal(Scalar H[][3], Scalar* m, CoordSetT& cs)
{
  ...
  for(int i = 0; i < 9; ++ i) {
    for(int j = 0; j < 9; ++j) {
      ...
      H[27*(3*j  )+3*i+1][0] = 0;
      H[27*(3*j  )+3*i+1][1] = 0;
      H[27*(3*j  )+3*i+1][2] = dShapex[j]*dShapey[i] - dShapex[i]*dShapey[j];

      H[27*(3*j  )+3*i+2][0] = 0;
      H[27*(3*j  )+3*i+2][1] = dShapex[i]*dShapey[j] - dShapex[j]*dShapey[i];
      H[27*(3*j  )+3*i+2][2] = 0;

      H[27*(3*j+1)+3*i  ][0] = 0;
      H[27*(3*j+1)+3*i  ][1] = 0;
      H[27*(3*j+1)+3*i  ][2] = dShapex[i]*dShapey[j] - dShapex[j]*dShapey[i];

      H[27*(3*j+1)+3*i+1][0] = 0;
      H[27*(3*j+1)+3*i+1][1] = 0;
      H[27*(3*j+1)+3*i+1][2] = 0;

      H[27*(3*j+1)+3*i+2][0] = dShapex[j]*dShapey[i] - dShapex[i]*dShapey[j];
      H[27*(3*j+1)+3*i+2][1] = 0;
      H[27*(3*j+1)+3*i+2][2] = 0;

      H[27*(3*j+2)+3*i  ][0] = 0;
      H[27*(3*j+2)+3*i  ][1] = dShapex[j]*dShapey[i] - dShapex[i]*dShapey[j];
      H[27*(3*j+2)+3*i  ][2] = 0;

      H[27*(3*j+2)+3*i+1][0] = dShapex[i]*dShapey[j] - dShapex[j]*dShapey[i];
      H[27*(3*j+2)+3*i+1][1] = 0;
      H[27*(3*j+2)+3*i+1][2] = 0;

      H[27*(3*j+2)+3*i+2][0] = 0;
      H[27*(3*j+2)+3*i+2][1] = 0;
      H[27*(3*j+2)+3*i+2][2] = 0;
    }
  }
}
...
template<typename Scalar, typename CoordSetT>
void
FaceQuad9::GetUnitNormal(Scalar Normal[3], Scalar *m, CoordSetT &cs)
{
