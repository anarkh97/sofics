C
C********************************************************************************
      SUBROUTINE ZPADE(c,ic,a,ia,b,ib,l,m,x,w,w1,w2,ik,iw)
C********************************************************************************
C
C     The complex function ZPADE returns the value of the [L/M] Pade
...
C
C     Complex double precision arrays w(iw,iw), w1(iw), w2(i2) and integer
...
C     at entry.
C
C     c is a complex array of dimension ic. On entry, it contains
C     the given series coefficients and is unchanged on exit.
C
C     a is a complex array of dimension ia. On exit, a(1), a(2), ..., a(l+1)
C     contain the numerator parameters a0, a1, ..., a(l), respectively
C
C     b is a complex array of dimension ib. On exit, b(1), b(2), ..., b(m+1)
...
C
C     w is a two-dimensional double precision complex array of dimensions (iw,iw)
C     which must be provided for workspace.
C
C     w1, w2 are two double precision complex arrays of dimension iw which must
...
C     zpade( ) = 0 is returned.
C
C********************************************************************************
C
      complex*16 c(ic),b(ib),a(ia),w(iw,iw),w1(iw),w2(iw)
      integer*4 ik(iw)
      real*8 x
      complex*16  one,ought,det,t,d,y,rpade
cc      data one,ought/0.1d 01,0.0d 00/
      one = (1.0,0.0)
      ought = (0.0,0.0)
C
...
15          do 18 k = 1,m
            if(ik(k) - 1) 16,18,30
16          if(cdabs(t) - cdabs(w(j,k))) 17,17,18
17          irow = j
            icol = k
            t = w(j,k)
18          continue
...
22    w2(i) = w(icol,icol)
      im1 = i-1
      if(i.eq.1) go to 25
      t = dexp(dlog(cdabs(det))/dble(float(im1)))
cc      if(cdabs(w2(i)) - t*dble(float(i))*eps) 23,25,25
      if(cdabs(w2(i)) - cdabs(t)*dble(float(i))*eps) 23,25,25
23    det = ought
...
C
      y = cdabs(d)/m
cc      if(y.lt.eps) go to 42
      if(cdabs(y).lt.eps) go to 42
35    if(l) 40,36,37
36    rpade = c(1)/d
      return
C
