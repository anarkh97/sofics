...
void
Penta26Corotator::getStiffAndForce(GeomState &geomState, CoordSet &cs,
                                   FullSquareMatrix &K, double *f, double dt, double t)
{
  int i,j,k;

  // initialize forces and stiffness matrix
  for (i = 0; i < 78; i++) f[i] = 0;
  K.zero();
  ...
  for(int n = 0; n < 18; n++) {
    ...
    dOmega = Penta26ShapeFct(Shape, nGrad, gauss3d9[n], X, Y, Z);
...
void
Penta26Corotator::getInternalForce(GeomState &geomState, CoordSet &cs,
...
double
Penta26Corotator::computeStrainGrad(GeomState &geomState, CoordSet &cs,
                                    double dedU[78][6], double m[3])
{
  int i, j;
  double dOmega; // det of jacobian
  double nGrad[26][3];
  ...
  double Shape[26];
  dOmega = Penta26ShapeFct(Shape, nGrad, m, X, Y, Z);
                                     ...
                                     double stress[26][7], double strain[26][7])
{
  ...
  for (n = 0; n < 26; n++) { // loop over nodes
    ...
    // compute shape functions
    Penta26ShapeFct(Shape, nGrad, nodeRefCoord[n], X, Y, Z);
...
void
Penta26Corotator::extractDeformations(GeomState &geomState, CoordSet &cs,
...
void
Penta26Corotator::extractRigidBodyMotion(GeomState &geomState, CoordSet &cs,
                                         double *vlr)
{
}

double
Penta26Corotator::getElementEnergy(GeomState &geomState, CoordSet &cs)
{
