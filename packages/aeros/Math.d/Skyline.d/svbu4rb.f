...
C=AUTHOR June 1996
C=BLOCK Force
      subroutine SVBU4RB(COLVAL,LD,LACOL,B,W,PIVOT,
     &                  TOL,NEQ,FLAG,NOPS,NZEM,ZEM)
C----------------------------------------------------------------------------
...
C
      IF(ABS(COLVAL(P1)).LE.TOL*ZEM(K,1)) THEN
         PIVOT(K)      = 0
...
C     Handle Singularities
C
      IF(ABS(COLVAL(P2)).LE.TOL*ZEM(K+1,1)) THEN
         PIVOT(K + 1) = 0
...
C     Handle Singularities
C
      IF(ABS(COLVAL(P3)).LE.TOL*ZEM(K+2,1)) THEN
         PIVOT(K + 2) = 0
...
C     Handle Singularities
C
      IF(ABS(COLVAL(P4)).LE.TOL*ZEM(K+3,1)) THEN
...
C        Handle Singularities
C
         IF(ABS(COLVAL(LD(NEQ - 2))).LE.TOL*ZEM(NEQ-2,1)) THEN
            PIVOT(NEQ - 2)      = 0
...
C        Handle Singularities
C
         IF(ABS(COLVAL(LD(NEQ - 1))).LE.TOL*ZEM(NEQ-1,1)) THEN
            PIVOT(NEQ - 1)      = 0
...
C        Handle Singularities
C
         IF(ABS(COLVAL(LD(NEQ))).LE.TOL*ZEM(NEQ,1)) THEN
            PIVOT(NEQ)      = 0
...
C=end force
C
      REAL*8 FUNCTION CHDDOT(LEN,A,B)
      REAL*8  A(1),B(1)
      INTEGER LEN
C
      REAL*8  S
