...
C=PURPOSE and loop is unrolled level 4
C=PURPOSE FORWARD and BACKWARD are serial and rolled -
C=PURPOSE If not empty, Null Space (ZERO ENERGY MODES) is computed 
C=AUTHOR  M. LESOINNE
C=AUTHOR  March 1998 
C=BLOCK Force
      subroutine pfactc(COLVAL,LD,LACOL,B,W,PIVOT,
     &                  TOL,NEQ,FLAG,NOPS,NZEM,ZEM,PROCID,
     &                  NPROC, BAR, ll)
C----------------------------------------------------------------------------
C     PFACTC =   skyline solver  using variable band information
C                and returning eventual ZERO ENERGY MODES.
C
C     Version 1:
C     ==========
C     FACTOR is fully parallelized
...
C
        IF(ZABS(COLVAL(P1)).LT.TOL*ZABS(ZEM(K,1))) THEN
           PIVOT(K)      = 0
...
C     Handle Singularities
C
        IF(ZABS(COLVAL(P2)).LT.TOL*ZABS(ZEM(K+1,1))) THEN
           PIVOT(K + 1) = 0
...
C     Handle Singularities
C
        IF(ZABS(COLVAL(P3)).LT.TOL*ZABS(ZEM(K+2,1))) THEN
           PIVOT(K + 2) = 0
...
C     Handle Singularities
C
        IF(ZABS(COLVAL(P4)).LT.TOL*ZABS(ZEM(K+3,1))) THEN
...
C        Handle Singularities
C
         IF(ZABS(COLVAL(LD(NEQ - 2))).LT.TOL*ZABS(ZEM(NEQ-2,1))) THEN
            PIVOT(NEQ - 2)      = 0
...
C        Handle Singularities
C
         IF(ZABS(COLVAL(LD(NEQ - 1))).LT.TOL*ZABS(ZEM(NEQ-1,1))) THEN
            PIVOT(NEQ - 1)      = 0
...
C        Handle Singularities
C
         IF(ZABS(COLVAL(LD(NEQ))).LT.TOL*ZABS(ZEM(NEQ,1))) THEN
            PIVOT(NEQ)      = 0
