...
************************************************************************
************************************************************************
*****   BLKL2 ... block general sparse Cholesky factorization      *****
************************************************************************
************************************************************************
*
*   --------
...
*     of The matrix can be semi-definite.  The computation is
...
************************************************************************
*
      SUBROUTINE  BLKL2   ( NPANEL, XPANEL, PNODE , XLINDX, LINDX ,
...
*       ------------------------
        EXTERNAL            ASSMB , DCOPY , DGECP , DGEMM , DPOTRF_LDL,
     &                      DSCAL , DTRSM , IGATHR, LDINDX, MMPYI
*
************************************************************************
*
        IFLAG = 0
...
*                   -------------------------------------------
                    ...
                    IWPTR = 1
                    DO  K = 1, NK
                        CALL  DCOPY ( NJ, LNZ(ILPTR), 1,
     &                                RWORK(IWPTR), 1 )
                        CALL  DSCAL ( NJ, LNZ(IDPTR), RWORK(IWPTR), 1 )
                        IDPTR = IDPTR + KPLEN + 1
                        ILPTR = ILPTR + KPLEN
                        IWPTR = IWPTR + NJ
                    END DO
...
*                       (with one column).
*                       ----------------------------------------
                        CALL  DCOPY ( NUPS, LNZ(KLPNT), 1,
     &                                RWORK, 1 )
                        CALL  DSCAL ( NUPS, LNZ(XLNZ(FK)),
     &                                RWORK, 1 )
                        CALL  MMPYI (
...
*
                            ...
                            IWPTR = 1
                            DO  K = 1, NK
                                CALL  DCOPY ( NUPS, LNZ(ILPTR), 1,
     &                                        RWORK(IWPTR), 1 )
                                CALL  DSCAL ( NUPS, LNZ(IDPTR),
     &                                        RWORK(IWPTR), 1 )
                                IDPTR = IDPTR + KPLEN + 1
                                ILPTR = ILPTR + KPLEN
                                IWPTR = IWPTR + NUPS
...
*               pivoting.
*               --------------------------------------------------
                CALL  DGECP ( NJ, LNZ(JLPNT), JLEN, TOL, ASDEF,
     &                        LBDEF,
     &                        IPROW, IPCOL, INFO )
                IF  ( INFO .NE. 0 )  THEN
                    IFLAG = 101
...
*               ---------------
     ...
     &              )
                ...
                JLPTR = JLPNT + NJ
                DO  J = FJ, LJ
                    CALL  DSCAL ( JLEN-NJ, ONE/LNZ(JDPTR),
     &                          LNZ(JLPTR), 1 )
                    JDPTR = JDPTR + JLEN + 1
                    JLPTR = JLPTR + JLEN
                END DO
