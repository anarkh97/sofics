...
************************************************************************
************************************************************************
*****   BLKSLV2 ... block triangular solutions 2 rhs               *****
************************************************************************
************************************************************************
*
*   --------
...
************************************************************************
*
      SUBROUTINE  BLKSLV2 ( NPANEL, XPANEL, XLINDX, LINDX , XLNZ  ,
...
*
        N = XPANEL(NPANEL+1) - 1
        DO  J = 1, N
            TEMP(1,J) = RHS(1,PERM(J))
            TEMP(2,J) = RHS(2,PERM(J))
            SOLN(1,J) = ZERO
            SOLN(2,J) = ZERO
        END DO
*
...
*
            ...
            DO  J = 1, JLEN-NJ
                JJ         = JJ + 1
                ISUB       = LINDX(JJ)
                TEMP(1,ISUB) = TEMP(1,ISUB) + SOLN(1,J)
                TEMP(2,ISUB) = TEMP(2,ISUB) + SOLN(2,J)
                SOLN(1,J)    = ZERO
                SOLN(2,J)    = ZERO
            END DO
*
...
*
        ...
        DO  JPANEL = 1, LSTPNL
            ...
            JDPTR = XLNZ(FJ)
            DO  J = FJ, LJ
                TEMP(1,J) = TEMP(1,J)/LNZ(JDPTR)
                TEMP(2,J) = TEMP(2,J)/LNZ(JDPTR)
                JDPTR   = JDPTR + JLEN + 1
            END DO
        END DO
*
...
*
            ...
            DO  J = 1, JLEN
                JJ      = JJ + 1
                ISUB    = LINDX(JJ)
                SOLN(1,J) = TEMP(1,ISUB)
                SOLN(2,J) = TEMP(2,ISUB)
            END DO
*
            IF  ( JLEN .GT. NJ )  THEN
                CALL  DGEMV (
...
*
        DO  J = 1, N
            SOLN(1,J) = TEMP(1,INVP(J))
            SOLN(2,J) = TEMP(2,INVP(J))
        END DO
*
        RETURN
*
