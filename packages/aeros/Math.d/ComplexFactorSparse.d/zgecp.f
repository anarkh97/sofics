...
*  =======
*
*  ZGECP computes the triangular factorization of a complex general matrix
...
*
            IF  ( K .NE. IROW )  THEN
                CALL  ZSWAP ( N, A(K,1), LDA, A(IROW,1), LDA )
            END IF
*
*           Interchange columns ...
*
            IF  ( K .NE. JCOL )  THEN
                CALL  ZSWAP ( N, A(1,K), 1, A(1,JCOL), 1 )
            END IF
*
            IF  ( K .LT. N )  THEN
*
*               Compute multipliers ...
*
                CALL  ZSCAL ( N-K, ONE/A(K,K), A(K+1,K), 1 )
*
*               Apply rank-1 update ...
*
                CALL  ZGERU( N-K, N-K, -ONE, A(K+1,K), 1, A(K,K+1),
     &                       LDA, A(K+1,K+1), LDA )
            END IF
*
        END DO
