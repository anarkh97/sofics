...
#endif
{
  ...
  for (i = 0; i < me; i++)
  {
    ...
    std::cerr << "r = " << r.head(iq).transpose() << std::endl;
    std::cerr << "d = " << d.transpose() << std::endl;
#endif
 
    /* compute full step length t2: i.e., the minimum step in primal space s.t. the contraint 
     * becomes feasible */
    t2 = 0.0;
    if (abs(z.dot(z)) > 0/*PJSA std::numeric_limits<double>::epsilon()*/) // i.e. z != 0
...
#ifdef TRACE_SOLVER
  std::cerr << "s = " << s.head(mi).transpose() << std::endl;
#endif

  if (abs(psi) <= mi * std::numeric_limits<double>::epsilon() * c1 * c2 * tol) // PJSA tol was originally 100.0
  {
    /* numerically there are not infeasibilities anymore */
    q = iq;
    if(lambda) for(int i=0; i<me; ++i) (*lambda)[-A(i)-1] = u[i];
    if(mu) { mu->setZero(); for (i=me; i<q; i++) (*mu)[A(i)] = u[i]; }
    return f_value;
  }
...
l2: /* Step 2: check for feasibility and determine a new S-pair */
  ...
  std::cerr << "np = " << np.transpose() << std::endl;
#endif
...
l2a:/* Step 2a: determine step direction */
  ...
  std::cerr << "z = " << z.transpose() << std::endl;
  std::cerr << "r = " << r.head(iq+1).transpose() << std::endl;
  std::cerr << "u = " << u.head(iq+1).transpose() << std::endl;
  std::cerr << "d = " << d.transpose() << std::endl;
  std::cerr << "A = " << A.head(iq+1).transpose() << std::endl;
#endif
  ...
  if (abs(t - t2) < std::numeric_limits<double>::epsilon()) //PJSA if (t == t2)
  {
#ifdef TRACE_SOLVER
...
#ifdef TRACE_SOLVER
      std::cerr << "R =\n" << R << std::endl;
      std::cerr << "A = " << A.head(iq).transpose() << std::endl;
#endif
      for (i = 0; i < m; i++)
        iai(i) = i;
      for (i = 0; i < iq; i++)
