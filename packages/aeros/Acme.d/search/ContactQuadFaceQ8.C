...
ContactQuadFaceQ8* ContactQuadFaceQ8::new_ContactQuadFaceQ8(
                        ContactFixedSizeAllocator* alloc,
                        int Block_Index, int Index_in_Block, int key)
{
  return new (alloc[ContactSearch::ALLOC_ContactQuadFaceQ8].New_Frag())
             ContactQuadFaceQ8(alloc, Block_Index, Index_in_Block, key);
}

void ContactQuadFaceQ8_SizeAllocator(ContactFixedSizeAllocator& alloc)
{
  alloc.Resize( sizeof(ContactQuadFaceQ8),
                100,  // block size
                0);  // initial block size
  alloc.Set_Name( "ContactQuadFaceQ8 allocator" );
}

ContactQuadFaceQ8::~ContactQuadFaceQ8() {}

void ContactQuadFaceQ8::Compute_Normal(VariableHandle CURRENT_POSITION,
...
void ContactQuadFaceQ8::Compute_Normal(VariableHandle POSITION,
                                       Real* normal,
				       Real* local_coords )
{
  Real shape_derivatives[2][8];
...
void ContactQuadFaceQ8::Compute_Normal(Real** nodal_positions,
...
void ContactQuadFaceQ8::Get_Edge_Nodes( int i, ContactNode<Real>** node )
{
  ...
  default:
    ...
    std::cerr << "Invalid Edge request in ContactQuadFaceQ8::Get_Face_Nodes" << std::endl;
#endif
    POSTCONDITION( 0 );
  }
}
...
int ContactQuadFaceQ8::Get_Edge_Number( Real* local_coords )
{
...
void 
ContactQuadFaceQ8::Compute_Edge_Normal( VariableHandle POSITION,
					VariableHandle FACE_NORMAL,
					int Edge,
					Real* edge_normal)
{
...
void ContactQuadFaceQ8::Get_Close_Edges( Real* local_coords, int& number,
...
bool ContactQuadFaceQ8::Is_Inside_Face( Real* local_coords )
{
  PRECONDITION( 0 );
  return false;
}
...
void ContactQuadFaceQ8::FacetStaticRestriction(int nfacets, 
                                               Real* coordinates, 
                                               Real* normals, 
                                               Real* ctrcl_facets, 
                                               Real* ctrcl)
...
void ContactQuadFaceQ8::FacetDynamicRestriction(int nfacets, 
                                                Real* ctrcl_facets, 
                                                Real* ctrcl)
{
  int i;
...
void ContactQuadFaceQ8::Smooth_Normal( VariableHandle CURRENT_POSITION,
                                       ...
                                       Real critical_curvature )
{
  ...
  Real local_coords[3];
  Compute_Local_Coords( node_position, contact_point, &(local_coords[0]));

  // Now interpolate to get the normal
  Interpolate_Vector( &(local_coords[0]), node_normal, smooth_normal );
...
void ContactQuadFaceQ8::Compute_Node_Areas( VariableHandle POSITION,
                                            VariableHandle FACE_NORMAL,
                                            Real* node_areas )
{
  // In the short term, I'm going to compute the node areas as if this were
  // a quad4 and then distribute the areas to the eight nodes
  ...
  // Now compute the areas for all eight nodes
  node_areas[4] = 0.25*( node_areas[0] + node_areas[1] );
  node_areas[5] = 0.25*( node_areas[1] + node_areas[2] );
  node_areas[6] = 0.25*( node_areas[2] + node_areas[3] );
  node_areas[7] = 0.25*( node_areas[3] + node_areas[0] );
  node_areas[0] *= 0.5;
  node_areas[1] *= 0.5;
  node_areas[2] *= 0.5;
  node_areas[3] *= 0.5;
...
void ContactQuadFaceQ8::Evaluate_Shape_Functions( Real* local_coords,
						  Real* shape_functions )
{
  Compute_Shape_Functions( local_coords, shape_functions );
}

void ContactQuadFaceQ8::Compute_Global_Coordinates( VariableHandle POSITION,
						    Real* local_coords,
						    Real* global_coords )
{
...
void ContactQuadFaceQ8::Compute_Local_Coordinates( Real Config_Param,
...
void ContactQuadFaceQ8::Compute_Local_Coordinates( VariableHandle POSITION,
						   Real* global_coords,
						   Real* local_coords )
{
  int  i, j;
...
void ContactQuadFaceQ8::Compute_Shape_Functions( Real local_coords[2],
						 Real shape_functions[8] )
{
  Real xi  = local_coords[0];
  Real eta = local_coords[1];
  shape_functions[0] = 0.25*(1.0-xi)*(1.0-eta)*(-xi-eta-1.0);
  shape_functions[1] = 0.25*(1.0+xi)*(1.0-eta)*( xi-eta-1.0);
  shape_functions[2] = 0.25*(1.0+xi)*(1.0+eta)*( xi+eta-1.0);
  shape_functions[3] = 0.25*(1.0-xi)*(1.0+eta)*(-xi+eta-1.0);
  shape_functions[4] = 0.50*(1.0-xi*xi)*(1.0-eta    );
  shape_functions[5] = 0.50*(1.0+xi   )*(1.0-eta*eta);
  shape_functions[6] = 0.50*(1.0-xi*xi)*(1.0+eta    );
  shape_functions[7] = 0.50*(1.0-xi   )*(1.0-eta*eta);
}

void ContactQuadFaceQ8::Compute_Shape_Derivatives( Real local_coords[2],
						   Real shape_derivatives[2][8] )
{
  Real xi  = local_coords[0];
  Real eta = local_coords[1];
  shape_derivatives[0][0] = -0.25*(1.0-eta)*(-2.0*xi-eta);
  shape_derivatives[0][1] =  0.25*(1.0-eta)*( 2.0*xi-eta);
  shape_derivatives[0][2] =  0.25*(1.0+eta)*( 2.0*xi+eta);
  shape_derivatives[0][3] = -0.25*(1.0+eta)*(-2.0*xi+eta);
  shape_derivatives[0][4] = -0.50*(2.0*xi )*(1.0-eta);
  shape_derivatives[0][5] =  0.50*          (1.0-eta*eta);
  shape_derivatives[0][6] = -0.50*(2.0*xi )*(1.0+eta);
  shape_derivatives[0][7] = -0.50*          (1.0-eta*eta);
  
  shape_derivatives[1][0] = -0.25*(1.0-xi)*(-2.0*eta-xi);
  shape_derivatives[1][1] = -0.25*(1.0+xi)*(-2.0*eta+xi);
  shape_derivatives[1][2] =  0.25*(1.0+xi)*( 2.0*eta+xi);
  shape_derivatives[1][3] =  0.25*(1.0-xi)*( 2.0*eta-xi);
  shape_derivatives[1][4] = -0.50*(1.0-xi*xi);
  shape_derivatives[1][5] = -0.50*(1.0+xi   )*(2.0*eta);
  shape_derivatives[1][6] =  0.50*(1.0-xi*xi);
  shape_derivatives[1][7] = -0.50*(1.0-xi   )*(2.0*eta);
}

void ContactQuadFaceQ8::Compute_Local_Coords( Real node_positions[MAX_NODES_PER_FACE][3],
...
void ContactQuadFaceQ8::Compute_Quad_Local_Coords( Real node_positions[MAX_NODES_PER_FACE][3],
					      Real global_coords[3],
					      Real local_coords[3] )
{
  int  i, j;
  ...
  if (!converged) {
    std::cerr << "ContactQuadFaceQ8::Compute_Local_Coords() did not converge" 
	 << std::endl;
...
void ContactQuadFaceQ8::Compute_Global_Coords( Real node_positions[8][3],
					       Real local_coords[2],
					       Real global_coords[3] )
{
  Real N[8];
...
void ContactQuadFaceQ8::Interpolate_Scalar( Real  local_coords[2],
					    Real  node_scalars[8],
...
void ContactQuadFaceQ8::Interpolate_Vector( Real local_coords[2],
