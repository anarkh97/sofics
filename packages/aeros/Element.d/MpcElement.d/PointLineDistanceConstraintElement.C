...
PointLineDistanceConstraintElement::PointLineDistanceConstraintElement(int* _nn)
		: ConstraintFunctionElement<Simo::PointLineDistanceConstraintFunction>(1, DofSet::XYZdisp, _nn, 0)
{
}

void
PointLineDistanceConstraintElement::setFrame(EFrame *elemframe)
{
	for(int i = 0; i < 3; ++i) {
		x1[i] = (*elemframe)[0][i];
		x2[i] = (*elemframe)[1][i];
...
void
PointLineDistanceConstraintElement::getConstants(const CoordSet & cs,
                                                 Eigen::Array<double,14,1>& sconst, Eigen::Array<int,1,1>& iconst,
