...
C=====================================================================C
      subroutine compbBM( elm , type , x     , y  , dbm  , fb   ,
     $                    clr , cqr  , alpha , fm , rowb , colm ,
     $                    rot , L    , P     , fastcal   , kbBM )
C=====================================================================C
C
C     -----------
C     DECLARATION
...
C
      ...
      real*8    x(3) , y(3) , dbm(3,3) , kbBM(18,18) , rot(6,6)
...
C
      ...
      real*8    dbmPt1 , dbmPt2 , dbmPt3 , factorB
C
C     ----
C     DATA
C     ----
...
C     -------------------------------------------------------------
C
C     STIFFNESS MATRIX ASSEMBLY FOR BASIC BENDING-MEMBRANE COUPLING
C
C     -------------------------------------------------------------
C
  900 continue
C
C.....ASSEMBLE THE OUTPUT STIFFNESS SUCH THAT [kbBM] = [L]*[dbm]*[P]^T
C
      do 5001 i=1,9
C
         col    = colm(i)
         dbmPt1 = dbm(1,1)*P(i,1) + dbm(1,2)*P(i,2) + dbm(1,3)*P(i,3)
         dbmPt2 = dbm(2,1)*P(i,1) + dbm(2,2)*P(i,2) + dbm(2,3)*P(i,3)
         dbmPt3 = dbm(3,1)*P(i,1) + dbm(3,2)*P(i,2) + dbm(3,3)*P(i,3)
C
         do 5002 j=1,9
            row           = rowb(j)
            kbBM(row,col) = kbBM(row,col)
     $                    + dbmPt1*L(j,1)
     $                    + dbmPt2*L(j,2)
     $                    + dbmPt3*L(j,3)
 5002    continue
...
C
*     open(unit=90,file="kbBM.m")
*     write(90,*) "kbBM=["
*     do 991 i=1,18
* 991 write(90,9) (kbBM(i,j),j=1,18)
*     write(90,*) "     ];"
*     close(90)
...
C
C      call compmrot( kbBM , rot , rot , rot )
C
C     ------
C     RETURN
C     ------
...
C
  100 continue
      write(*,*) "*** FATAL ERROR in routine COMPBBM      ***"
