...
C=====================================================================C
      subroutine comphBM( elm , type , x    , y   , dbm     , fb   ,
     $                    fm  , rowb , colm , rot , Lh1     , Lh2  ,
     $                    Lh3 , Ph1  , Ph2  , Ph3 , fastcal , khBM )
C=====================================================================C
C
C     -----------
C     DECLARATION
...
C
      col    = colm(i)
C
      mult11 = dbm(1,1)*Ph1(i,1) + dbm(1,2)*Ph1(i,2) + dbm(1,3)*Ph1(i,3)
      mult12 = dbm(2,1)*Ph1(i,1) + dbm(2,2)*Ph1(i,2) + dbm(2,3)*Ph1(i,3)
      mult13 = dbm(3,1)*Ph1(i,1) + dbm(3,2)*Ph1(i,2) + dbm(3,3)*Ph1(i,3)
C
      mult21 = dbm(1,1)*Ph2(i,1) + dbm(1,2)*Ph2(i,2) + dbm(1,3)*Ph2(i,3)
      mult22 = dbm(2,1)*Ph2(i,1) + dbm(2,2)*Ph2(i,2) + dbm(2,3)*Ph2(i,3)
      mult23 = dbm(3,1)*Ph2(i,1) + dbm(3,2)*Ph2(i,2) + dbm(3,3)*Ph2(i,3)
C
      mult31 = dbm(1,1)*Ph3(i,1) + dbm(1,2)*Ph3(i,2) + dbm(1,3)*Ph3(i,3)
      mult32 = dbm(2,1)*Ph3(i,1) + dbm(2,2)*Ph3(i,2) + dbm(2,3)*Ph3(i,3)
      mult33 = dbm(3,1)*Ph3(i,1) + dbm(3,2)*Ph3(i,2) + dbm(3,3)*Ph3(i,3)
C
      do 6002 j=1,9
         row           = rowb(j)
         khBM(row,col) = khBM(row,col)   + mult11*Lh1(j,1)
     $                 + mult12*Lh1(j,2) + mult13*Lh1(j,3)
     $                 + mult21*Lh2(j,1) + mult22*Lh2(j,2)
     $                 + mult23*Lh2(j,3) + mult31*Lh3(j,1)
     $                 + mult32*Lh3(j,2) + mult33*Lh3(j,3)
 6002 continue
C
 6001 continue
C
...
C
*     open(unit=90,file="khBM.m")
*     write(90,*) "khBM=["
*     do 991 i=1,18
* 991 write(90,9) (khBM(i,j),j=1,18)
*     write(90,*) "     ];"
*     close(90)
