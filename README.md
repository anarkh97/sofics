...
`M2C` and `Aero-S` are in-house, open-source solvers developed in C++, while `Gmsh` ([Geuzaine *et al.*](https://doi.org/10.1002/nme.2579)) and `Dakota` ([Adams *et al.*](https://doi.org/10.2172/1817318)) are external open-source tools. `M2C` includes a non-uniform Cartesian mesh generator, allowing users to define the finite-volume mesh for fluid dynamics directly through the `M2C` input file (see examples in the `examples` directory). As a result, `Gmsh` is only needed to create meshes for structural dynamics analysis.

A C++ utility called `gmsh2aeros` is available in the `src/tools` directory. This utility converts finite-element meshes from the `MSH` format (used by `Gmsh`) to the `TOP` format required by `Aero-S`. The utility is built along with `SOFICS`.
...
A C++ utility called `aeros2dakota` is available in the `src/tools` directory. This utility converts the results from `Aero-S` to relevant function values. The utility is built along with `SOFICS`.
...
The external libraries that `M2C` and `Aero-S` link against (MPI, BLAS/LAPACK, PETSc, MUMPS, and so on) are covered in [INSTALL_DEPENDENCIES.md](INSTALL_DEPENDENCIES.md). Please install those before building.
...
![Aero-S default output.](media/AeroSOutput.jpg)
...
![M2C default output.](media/M2COutput.jpg)
...
```text
analysis_driver = 'path/to/sofics/build/sofics/bin/driver.sh --config path/to/config.sh --pre path/to/pre_pro.sh --post path/to/post_pro.sh'
```
...
![Gmsh test output.](media/GmshOutput.jpg)
...
![Dakota test output.](media/DakotaOutput.jpg)
