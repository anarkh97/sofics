...
This example case demonstrates the simulation setup required for successful evaluation of two structural design configurations using coupled fluid-structure interaction simulations. The provided `config.sh` is sized for a workstation, allocating 4 computational cores to each simulation. On a compute cluster, each simulation utilizes 64 computational cores: 56 for the fluid solver and 8 for the structural solver, and the simulations are executed asynchronously on a single compute node, through the `SLURM` scheduling directive.
...
***Note:*** The fluid mesh and the simulation setup are identical to the ones used on the compute cluster, so each evaluation takes considerably longer here than it does on 64 cores. The local setup is meant for verifying that the toolchain is configured correctly.

## Cluster Evaluation

To scale the study up for a compute cluster, set `M2C_SIZE=56` and `AEROS_SIZE=8` in `config.sh`, which allocates 64 computational cores to each simulation, and set `evaluation_concurrency = 2` in `dakota.in` so that both designs are evaluated asynchronously.
...
<!-- TODO: Paste the dakota.dat contents from a reference run here, so that
     users have something concrete to compare their output against. -->

Each evaluation directory also retains its inputs and outputs, since `dakota.in` requests `file_save` and `directory_save`. The structural mesh generated by `Gmsh` for a design is kept in `evaluation.N/StructModel`, and the fluid and structural solver output in `evaluation.N/results`.
